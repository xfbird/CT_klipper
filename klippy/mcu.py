...
class MCU:
    ...
    def _record_htop_info(self):
        from subprocess import check_output
        cmd = "echo q | htop -C > /dev/stdout"
        logging.error(cmd)
        htop = check_output(cmd, shell=True).decode("utf-8")
        logging.error(htop)
    ...
    def _handle_shutdown(self, params):
        ...
        if msg == "Timer too close":
            ...
            import threading
            t = threading.Thread(target=self._record_htop_info)
            t.start()
        ...
        self._printer.invoke_async_shutdown("""{"code": "%s", "msg":"%s", "values": []}""" % (code_key_string, prefix + msg + error_help(msg)))
    ...
    def usb_reset(self):
        ...
        if usb_id == "/dev/serial/by-id/usb_serial_1":
            ...
            return
        logging.info(check_dev_model_shell)
        ret = check_output(check_dev_model_shell, shell=True)
        logging.info(ret.strip().decode('utf-8'))
        if "# !CR-10 V3" in ret.strip().decode('utf-8'):
            logging.info(dev_check_shell)
